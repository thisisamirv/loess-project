test_that("fastloess() basic functionality works", {
    x <- c(1, 2, 3, 4, 5)
    y <- c(2, 4, 6, 8, 10)

    result <- fastloess(x, y, fraction = 0.67)
...
test_that("fastloess() handles different fractions", {
    set.seed(42)
    x <- seq(0, 10, length.out = 50)
    y <- sin(x) + rnorm(50, sd = 0.1)

    result_low <- fastloess(x, y, fraction = 0.2)
    result_high <- fastloess(x, y, fraction = 0.8)
...
test_that("fastloess() robustness iterations work", {
    set.seed(123)
    x <- seq(0, 10, length.out = 50)
    y <- sin(x) + rnorm(50, sd = 0.1)
    ...
    result_no_robust <- fastloess(x, y, fraction = 0.3, iterations = 0)
    result_robust <- fastloess(x, y, fraction = 0.3, iterations = 5)

    expect_equal(length(result_no_robust$y), length(y))
    expect_equal(length(result_robust$y), length(y))
})

test_that("fastloess() confidence intervals work", {
    set.seed(42)
    x <- seq(0, 10, length.out = 50)
    y <- sin(x) + rnorm(50, sd = 0.2)

    result <- fastloess(x, y, fraction = 0.5, confidence_intervals = 0.95)
...
test_that("fastloess() prediction intervals work", {
    set.seed(42)
    x <- seq(0, 10, length.out = 50)
    y <- sin(x) + rnorm(50, sd = 0.2)

    result <- fastloess(x, y, fraction = 0.5, prediction_intervals = 0.95)
    ...
    result_ci <- fastloess(x, y, fraction = 0.5, confidence_intervals = 0.95)
    expect_true(
...
test_that("fastloess() diagnostics work", {
    set.seed(42)
    x <- seq(0, 10, length.out = 50)
    y <- 2 * x + rnorm(50, sd = 0.5)

    result <- fastloess(x, y, fraction = 0.5, return_diagnostics = TRUE)
...
test_that("fastloess() residuals work", {
    set.seed(42)
    x <- seq(0, 10, length.out = 50)
    y <- sin(x) + rnorm(50, sd = 0.1)

    result <- fastloess(x, y, fraction = 0.5, return_residuals = TRUE)
...
test_that("fastloess() robustness weights work", {
    set.seed(42)
    x <- seq(0, 10, length.out = 50)
    y <- sin(x) + rnorm(50, sd = 0.1)
...
test_that("fastloess() cross-validation works", {
    set.seed(42)
...
test_that("fastloess() handles edge cases", {
    # Minimum data points
    x <- c(1, 2, 3)
    y <- c(1, 2, 3)
    result <- fastloess(x, y, fraction = 0.67)
    ...
    x <- 1:10
    y <- rep(5, 10)
    result <- fastloess(x, y, fraction = 0.5)
    expect_true(all(abs(result$y - 5) < 1e-10))
})

test_that("fastloess() parameter validation works", {
    x <- 1:10
    y <- 1:10

    # Mismatched lengths
    expect_error(fastloess(1:10, 1:5))
})

test_that("fastloess() parallel execution works", {
    set.seed(42)
    x <- seq(0, 10, length.out = 1000)
    y <- sin(x) + rnorm(1000, sd = 0.1)

    result_serial <- fastloess(x, y, fraction = 0.3, parallel = FALSE)
    result_parallel <- fastloess(x, y, fraction = 0.3, parallel = TRUE)
