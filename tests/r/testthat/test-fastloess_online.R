test_that("fastloess_online() basic functionality works", {
    set.seed(42)
    x <- 1:100
    y <- sin(x / 10) + rnorm(100, sd = 0.1)

    result <- fastloess_online(x, y,
...
test_that("fastloess_online() window capacity works", {
    set.seed(42)
    x <- 1:100
    y <- sin(x / 10) + rnorm(100, sd = 0.1)

    result_small <- fastloess_online(x, y,
        fraction = 0.3, window_capacity = 15
    )
    result_large <- fastloess_online(x, y,
...
test_that("fastloess_online() min_points parameter works", {
    set.seed(42)
    x <- 1:50
...
test_that("fastloess_online() update modes work", {
    set.seed(42)
    x <- 1:100
    y <- sin(x / 10) + rnorm(100, sd = 0.1)

    result_full <- fastloess_online(x, y,
        fraction = 0.3, window_capacity = 25,
        update_mode = "full"
    )
    result_incr <- fastloess_online(x, y,
        fraction = 0.3, window_capacity = 25,
...
test_that("fastloess_online() handles edge cases", {
    # Minimum data points
    x <- 1:10
    y <- 1:10
    result <- fastloess_online(x, y,
    ...
    result2 <- fastloess_online(x, y,
        fraction = 0.5, window_capacity = 20, min_points = 3
    )
    expect_equal(length(result2$y), 10)
})

test_that("fastloess_online() robustness works", {
    set.seed(42)
    x <- 1:100
    y <- sin(x / 10) + rnorm(100, sd = 0.1)
    y[50] <- y[50] + 5 # Add outlier

    result_no_robust <- fastloess_online(x, y,
        fraction = 0.3, window_capacity = 25,
        iterations = 0
    )
    result_robust <- fastloess_online(x, y,
        fraction = 0.3, window_capacity = 25,
        iterations = 3
    )
