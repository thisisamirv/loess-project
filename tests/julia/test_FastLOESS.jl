...
@testset "fastloess Julia Bindings" begin
	...
	@testset "Parameter Coverage" begin
		...
		@testset "Loess: return_se" begin
			r = fit(Loess(fraction = 0.5, return_se = true, surface_mode = "direct"), x20, y20)
		...
		@testset "StreamingLoess: minkowski via distance_metric string" begin
			...
			ylong = sin.(xlong)
			s = StreamingLoess(
				fraction = 0.3, chunk_size = 60, distance_metric = "minkowski:2.5",
			)
			r1 = process_chunk(s, xlong, ylong)
			r2 = finalize(s)
		...
		@testset "OnlineLoess: minkowski via distance_metric string" begin
			...
			yo = xo .* 2.0
			o = OnlineLoess(
				fraction = 0.5, window_capacity = 10, distance_metric = "minkowski:3",
			)
			r = add_points(o, xo, yo)
			@test length(r.y) == 20
		end
