//! GPU-accelerated execution engine for LOESS smoothing.
//!
//! This module provides the GPU-accelerated smoothing function for LOESS
...
use loess_rs::internals::api::LoessError;
use loess_rs::internals::engine::executor::{IterationResult, LoessConfig};
...
// Perform a GPU-accelerated LOESS fit pass.
pub fn fit_pass_gpu<T>(
    x: &[T],
    y: &[T],
    config: &LoessConfig<T>,
) -> Result<IterationResult<T>, LoessError>
where
    T: Float + Debug + Send + Sync + 'static,
{
    #[cfg(feature = "gpu")]
    {
        ...
        if guard.is_none() {
            match block_on(GpuExecutor::new()) {
                ...
                Err(e) => return Err(LoessError::RuntimeError(format!("GPU init failed: {}", e))),
            }
        }
