//! User guide and API reference for the `fastLoess` crate.
//!
//! Browse the submodules below for conceptual guides and worked examples.
//! For the API itself, see [`prelude`](crate::prelude).

/// Getting started: concepts, installation, and a quickstart guide.
#[cfg(doc)]
pub mod introduction {
    /// Conceptual introduction: what LOESS is and how it works.
    pub mod concepts {
...
/// User guide: adapters, intervals, and cross-validation.
#[cfg(doc)]
pub mod guide {
...
/// Advanced topics: boundary handling, merge strategies, degree, and dimensions.
#[cfg(doc)]
pub mod advanced {
    ...
    pub mod degree {
        #![doc = include_str!("../docs/advanced/degree.md")]
    }
    /// Multivariate LOESS: smoothing over multiple predictor dimensions.
    pub mod dimensions {
        #![doc = include_str!("../docs/advanced/dimensions.md")]
    }
}
...
pub mod api {
    ...
    /// `StreamingLoess` API reference.
    pub mod streaming {
        #![doc = include_str!("../docs/api/api-streaming.md")]
    }
    /// `OnlineLoess` API reference.
    pub mod online {
