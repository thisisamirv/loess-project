...
pub use crate::primitives::buffer::LoessBuffer;
...
impl<T: Float> LoessExecutor<T> {
    ...
    ) -> Result<ExecutorOutput<T>, LoessError>
    where
    ...
    pub fn run(
        ...
        x: &[T],
        y: &[T],
        buffer: Option<&mut LoessBuffer<T>>,
    ) -> Result<ExecutorOutput<T>, LoessError>
    where
        T: Float + WLSSolver + Debug + Send + Sync + 'static,
    {
        ...
        T: Float + WLSSolver + Debug + Send + Sync + 'static,
    {
        ...
        let buffers = if let Some(b) = buffer {
            ...
            internal_buffers = LoessBuffer::with_capacity(n);
