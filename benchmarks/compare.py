...
def load_all_data(output_dir: Path):
    files = {
        ...
        "rfastloess (Serial)": output_dir / "rfastloess_serial.json",
        "rfastloess (Parallel)": output_dir / "rfastloess_parallel.json",
    }
...
def plot_benchmarks(data: dict, plots_dir: Path) -> None:
    ...
    all_systems = ["R", "rfastloess (Serial)", "rfastloess (Parallel)"]
    systems = [s for s in all_systems if any(data.get(s, {}).values())]

    SYS_COLORS = {
        "R": "#4878d0",
        "rfastloess (Serial)": "#ee854a",
        "rfastloess (Parallel)": "#6acc65",
    }
...
def main():
    ...
    candidates = ["rfastloess (Serial)", "rfastloess (Parallel)"]
    baseline_label = "R"
    ...
    # Table Header
    print(
        f"{'Name':<20} | {baseline_label:^10} | {'rfastloess (Serial)':^21} | {'rfastloess (Parallel)':^21} |"
    )
    print("-" * 82)
