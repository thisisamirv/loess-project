...
fn main() -> Result<(), LoessError> {
    ...
    let mut processor = StreamingLoess::new().fraction(0.3f64).chunk_size(50usize).overlap(10usize).build()?;
...
See [rust.md](rust.md) for the full `LoessResult<T>` field reference.
