...
API reference for the `fastloess` native Node.js package.
...
The `fastloess` package uses a common configuration object for its smoothing functions.
...
const processor = new StreamingLoess(smoothingOptions, {
  chunkSize: 5000,
  overlap: 500
});
...
const online = new OnlineLoess(smoothingOptions, {
  windowCapacity: 100,
  minPoints: 2,
