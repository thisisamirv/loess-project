...
| `enp` | number \| null | Equivalent number of parameters (if `returnSe`) |
| `traceHat` | number \| null | Trace of hat matrix (if `returnSe`) |
| `delta1` | number \| null | First delta statistic (if `returnSe`) |
| `delta2` | number \| null | Second delta statistic (if `returnSe`) |
| `residualScale` | number \| null | Residual scale estimate (if `returnSe`) |
| `leverage` | Float64Array \| null | Per-point hat-matrix diagonal (if `returnSe`) |
