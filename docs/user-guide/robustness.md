...
=== "R"
    ```r
    result <- Loess(iterations = 3, robustness_method = "bisquare")$fit(x, y)
    ```
...
=== "R"
    ```r
    result <- Loess(iterations = 3, robustness_method = "huber")$fit(x, y)
    ```
...
=== "R"
    ```r
    result <- Loess(iterations = 3, robustness_method = "talwar")$fit(x, y)
    ```
...
=== "R"
    ```r
    result <- Loess(iterations = 5, return_robustness_weights = TRUE)$fit(x, y)
...
=== "R"
    ```r
    result <- Loess(iterations = 10, auto_converge = 1e-6)$fit(x, y)
    ```
