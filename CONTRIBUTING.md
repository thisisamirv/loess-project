...
The Batch adapter already covers a comprehensive set of options (polynomial degree, nD distance metrics, interpolation surface mode, robustness, cross-validation, intervals, custom weights, etc.), but there's still room to grow. Contributions in any of these areas are especially welcome:
...
- **Concurrent chunk processing**: `parallel` currently only parallelizes work *within* a single chunk (fitting, CV, intervals, KD-tree build, and vertex passes are all parallelized per-chunk); independent chunks still go through `process_chunk()` one at a time. A `process_chunks(&[...])` helper that processes chunks concurrently (carefully handling overlap-merge ordering) could meaningfully speed up large offline chunked jobs.
- **Checkpointable/resumable state**: the streaming buffer/builder state has no serialization support, so a long-running streaming session can't be saved and resumed across process restarts — useful for production ETL pipelines processing files far larger than memory over multiple runs.
...
make loess-rs          # Build only (cargo build)
make loess-rs-dev      # Format, lint, build, test, examples, doc snippets
make loess-rs-coverage # Run coverage
make loess-rs-clean    # Clean build artifacts

# fastLoess crate (high-level API with adapters)
make fastLoess          # Build only (cargo build)
make fastLoess-dev      # Format, lint, build, test, doc snippets
make fastLoess-coverage # Run coverage
make fastLoess-clean    # Clean build artifacts
```
...
│   ├── julia/            # C-API + Julia wrapper (FastLOESS.jl)
...
- Scopes are optional but helpful (e.g., `loess-rs`, `fastLoess`, `python`, `r`, `julia`, `nodejs`, `wasm`, `cpp`, `go`, `java`, `docs`).
...
```bash
# Rust tests
cargo test -p loess-rs --features dev
cargo test -p fastLoess --features dev
...
R_LIBS_USER=bindings/r/.r-lib Rscript -e \
  "Sys.setenv(NOT_CRAN='true'); testthat::test_dir('bindings/r/tests/testthat', package = 'rfastloess')"

# Julia tests
julia --project=bindings/julia/julia bindings/julia/tests/test_FastLOESS.jl
...
# C++ tests (Linux; requires prior: cargo build -p fastloess-cpp --profile release-c)
mkdir -p bindings/cpp/tests/build
cd bindings/cpp/tests/build
cmake -DFASTLOESS_LIB="$(pwd)/../../../target/release-c/libfastloess_cpp.so" \
      -DFASTLOESS_LIB_DIR="$(pwd)/../../../target/release-c" ..
make && ./test_fastloess_suite

# Go tests (requires prior: cargo build -p fastloess-go --profile release-c)
cd bindings/go/fastloess && go test ./... -v

# Java tests (requires prior: cargo build -p fastloess-java)
