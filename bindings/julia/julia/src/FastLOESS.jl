...
mutable struct Loess
	...
	function Loess(;
		...
		handle = @ccall libfastloess.jl_loess_new(
			...
			Cint(return_se)::Cint,
			(weighted_metric_weights !== nothing ? pointer(weighted_metric_weights) : Ptr{Cdouble}(C_NULL))::Ptr{Cdouble},
			Culong(weighted_metric_weights !== nothing ? length(weighted_metric_weights) : 0)::Culong,
		)::Ptr{Cvoid}
...
function fit(
	...
	c_result = @ccall libfastloess.jl_loess_fit(
		...
		y::Ptr{Cdouble},
		Culong(n)::Culong,
		(custom_weights !== nothing ? pointer(custom_weights) : Ptr{Cdouble}(C_NULL))::Ptr{Cdouble},
...
- `parallel::Bool = false`: Enable parallel execution (default `false` for online mode;
  online LOESS processes one point at a time and rarely benefits from parallelism)
- `degree::String = "linear"`: Polynomial degree
- `dimensions::Int = 1`: Number of predictor dimensions
- `distance_metric::String = "normalized"`: Distance metric ("normalized", "euclidean",
  "manhattan", "chebyshev", "minkowski"). Use "minkowski:p" for a custom p value.
...
function add_point(o::OnlineLoess, x::Float64, y::Float64)
	...
	if c_result.error != Ptr{Cchar}(C_NULL)
		error_msg = unsafe_string(Ptr{UInt8}(c_result.error))
		@ccall libfastloess.jl_online_free_output(Ref(c_result)::Ptr{CJlOnlineOutput})::Cvoid
		error("fastloess error: $error_msg")
	end
