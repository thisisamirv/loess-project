...
const LIBNAME = Sys.iswindows() ? "fastloess_jl.dll" :
...
function find_library()
    ...
    try
        @eval using fastloess_jll
        if @isdefined(fastloess_jll) && hasproperty(fastloess_jll, :libfastloess_jl)
            return fastloess_jll.libfastloess_jl
        end
    catch
...
"""
    smooth(x, y; kwargs...) -> LoessResult

Perform LOESS smoothing on the data using batch processing.
...
# Returns
- `LoessResult`: Smoothed values and optional diagnostics
...
function smooth(x::Vector{Float64}, y::Vector{Float64};
    ...
    c_result = @ccall libfastloess.jl_loess_smooth(
        x::Ptr{Cdouble},
        y::Ptr{Cdouble},
        Culong(n)::Culong,
        fraction::Cdouble,
...
"""
    smooth_streaming(x, y; kwargs...) -> LoessResult

Perform streaming LOESS smoothing for large datasets.
...
function smooth_streaming(x::Vector{Float64}, y::Vector{Float64};
    ...
    c_result = @ccall libfastloess.jl_loess_streaming(
        x::Ptr{Cdouble},
        y::Ptr{Cdouble},
        Culong(n)::Culong,
        fraction::Cdouble,
...
"""
    smooth_online(x, y; kwargs...) -> LoessResult

Perform online LOESS smoothing with a sliding window.
...
function smooth_online(x::Vector{Float64}, y::Vector{Float64};
    ...
    c_result = @ccall libfastloess.jl_loess_online(
        x::Ptr{Cdouble},
        y::Ptr{Cdouble},
        Culong(n)::Culong,
        fraction::Cdouble,
