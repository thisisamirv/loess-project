...
* Renamed Python's `guide/adapters.md` to `guide/adapter-choice.md` and `use-case/{genomics,real-time,time-series}.md` to `use-case/use-case-{genomics,real-time,time-series}.md`, matching the file-naming convention already used by every other binding/crate; updated the toctree entries and cross-reference links accordingly.
* Added the missing "- [Tutorials](quickstart.md)" line to C++ and Julia's `introduction/concepts.md` "Next Steps" section, matching every other binding/crate.
* Added the "Multivariate predictors" and "Adjustable polynomial degree" rows to the "Key Advantages" table in `introduction/concepts.md` for C++, Julia, Node.js, WASM, Python, `loess-rs`, and `fastLoess` — R's vignette already had these two LOESS-specific rows (absent from LOWESS, whose fork this repo split from), but they were never propagated to the other six sources.
...
* Fixed `bindings/cpp/docs/guide/intervals.md`'s "Prediction Intervals" section actually being a copy-paste of the "Confidence Intervals" section's code and output (it read `confidence_lower()`/`confidence_upper()`, never `prediction_lower()`/`prediction_upper()`); now computes and prints the real prediction bounds. Also fixed the same file's "Confidence Levels" section mislabeling its 99% CI output as "95% CI".
* Fixed `bindings/python/docs/guide/intervals.md`'s "Confidence Levels" section printing a `"95% CI at midpoint"` label for a `confidence_intervals=0.90` example; relabeled to "90% CI at midpoint".
* Fixed `bindings/python/docs/guide/cross-validation.md`'s "Interpreting Results" example computing a fit but never printing anything, unlike every other binding's equivalent section; added a `print(f"Selected fraction: {result.fraction_used}")`.
* Added the "Feature Comparison" table and "Next Steps" links to `guide/adapter-choice.md` for C++, Node.js, WASM, `loess-rs`, and `fastLoess`, matching the content already present in this repo's own Julia/Python docs and in every one of lowess-project's equivalent `adapter-choice.md` pages — these five had silently ended right after the adapter-comparison image.
