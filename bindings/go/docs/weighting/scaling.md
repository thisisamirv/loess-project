...
When `Iterations > 0`, Loess computes robustness weights by comparing each residual to the current residual scale estimate. `ScalingMethod` controls how that scale is measured.
