...
cmake --install build-cpp --config Release --prefix "$env:LOCALAPPDATA/fastloess"
```
...
If your environment disables the CMake package registry, use an installed prefix and set `CMAKE_PREFIX_PATH`, or set `fastloess_DIR` directly to the directory containing `fastloessConfig.cmake`.
