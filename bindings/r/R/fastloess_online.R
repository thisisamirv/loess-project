...
#'
#' @description
#' Sliding window LOESS for real-time data streams. Maintains a window of
#' recent points for incremental updates.
#'
#' For full documentation, see: \url{https://loess.readthedocs.io/}
#'
...
#' result <- fastloess_online(x, y, window_capacity = 25L)
#' plot(x, y)
#' lines(result$x, result$y, col = "red", lwd = 2)
#'
#' @seealso \code{\link{fastloess}}, \code{\link{fastloess_streaming}}
#' @family fastloess
#' @export
fastloess_online <- function(
    x,
    y,
    fraction = 0.2,
