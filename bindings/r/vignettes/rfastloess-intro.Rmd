...
`rfastloess` is a high-performance R package for LOESS (Locally Estimated Scatterplot Smoothing) built on a Rust backend.
...
# Plot
plot(x, y, pch = 16, col = "gray", main = "Basic LOESS Smoothing")
...
result_robust <- fastloess(x, y_outliers,
    fraction = 0.3,
    iterations = 5,
...
| Function                 | Use Case                             |
|--------------------------|--------------------------------------|
| `fastloess()`           | Primary interface - batch processing |
| `fastloess_streaming()` | Large datasets (>100K points)        |
| `fastloess_online()`    | Real-time data streams               |
