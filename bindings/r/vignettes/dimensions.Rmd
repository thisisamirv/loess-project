...
Standard LOESS operates on a single predictor `x`. Setting `dimensions > 1`
extends the neighbourhood search and local polynomial fit into an
**n-dimensional predictor space**, enabling surface smoothing over spatial
grids, time–altitude combinations, and similar multi-predictor datasets.

![Multivariate LOESS](../man/figures/multivariate_loess.svg)
...
> **Note:** Neighbourhood search scales with the number of dimensions. For `d ≥
> 3` keep `fraction` small and consider the `"interpolation"` surface mode (the
> default) to reduce computation.
...
Two predictors (e.g., latitude/longitude, time/altitude). Pass an `n × 2` matrix
as `x`.
...
Pass an `n × d` matrix as `x`. Higher dimensions require a larger `fraction` to
keep the local neighbourhood populated.
...
For large 2D or 3D datasets use `"interpolation"` (default) and tune `cell` and
`interpolation_vertices` to control grid resolution.
