...
Cross-validation helps select optimal parameters (especially `fraction`) by
...
> **Recommendation:** Use **5-fold** or **10-fold** CV for most applications.
> LOOCV is only worth it for small datasets (N < 100).
...
`result$cv_scores` contains the CV score for each candidate fraction in the
order they were passed. The fraction with **lowest CV score** is
