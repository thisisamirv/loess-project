...
# Python: custom-weights.md extra vars (calibration_indices / sigma / Loess)
...
_JS_STD = """\
const {{ {imports} }} = require('fastloess');
...
_WASM_STD_REQUIRE = "./fastloess_wasm.js"
...
def transform_julia(code: str, doc_rel: str) -> str | None:
    ...
    has_using = "using FastLOESS" in code
    ...
    """Detect which fastloess symbols are used in a Node.js snippet."""
    names = []
    for sym in ("Loess", "StreamingLoess", "OnlineLoess", "smooth"):
...
def transform_python(code: str, doc_rel: str) -> str | None:
    ...
    if "import fastloess" in code or "from fastloess" in code:
        return (
    ...
    if "positions" in code or "coverage" in code:
        ...
        "calibration_indices" in code
        or "sigma" in code
        or (re.search(r"\bLoess\s*\(", code) and "fl." not in code)
    ):
...
def _js_add_missing_data(code: str, doc_rel: str) -> str | None:
    ...
    # Fix: snippet uses fl. but fl is not bound (e.g. const { Loess } = require(...))
    if re.search(r"\bfl\.", code) and not re.search(r"\bconst\s+fl\s*=", code):
        code = re.sub(
            r"const\s*\{[^}]+\}\s*=\s*require\('fastloess'\);",
            "const fl = require('fastloess');",
            code,
        )
...
def transform_nodejs(code: str, doc_rel: str) -> str | None:
    ...
    if "xChunk" in code or "yChunk" in code:
        ...
        preamble = "const fl = require('fastloess');\n\n"
        if need_data:
            preamble += (
                "const n = 100;\n"
                "const x = Float64Array.from({ length: n }, (_, i) => i * 2 * Math.PI / (n - 1));\n"
    ...
    elif "fastloess." in code:
        # Code uses fastloess.Xxx namespace
        need_data = re.search(r"\bx\b|\by\b", code) and not re.search(
            r"(?:const|let|var)\s+[xy]\s*[=[]", code
        )
        preamble = "const fastloess = require('fastloess');\n\n"
        if need_data:
            preamble += (
                "const n = 100;\n"
                "const x = Float64Array.from({ length: n }, (_, i) => i * 2 * Math.PI / (n - 1));\n"
        ...
        if re.search(r"(?:const|let|var)\s+[xnt]\s*=", code, re.MULTILINE):
            return (
                f"const {{ {', '.join(imports)} }} = require('fastloess');\n\n" + code
            )
...
def transform_wasm(code: str, doc_rel: str) -> str | None:
    ...
    for m in re.finditer(
        r"^import\s+(?:init\s*,\s*)?\{([^}]+)\}\s+from\s+['\"]fastloess[_-]?wasm['\"];?",
        code,
        re.MULTILINE,
    ):
    ...
    # Also handle: import { Loess } from "fastloess_wasm.js"
    for m in re.finditer(
...
def transform_rust(code: str, doc_rel: str) -> str | None:
    ...
    top_uses: list[str] = []
    if not has_prelude:
        top_uses.append("use fastLoess::prelude::*;")
    ...
    result_parts = ["\n".join(top_uses), ""]
    result_parts.append("fn main() -> Result<(), LoessError> {")
...
_FIXABLE_REASONS = {
    "fastloess not imported (snippet is not self-contained)",
    "no require() \u2014 snippet must load fastloess itself",
