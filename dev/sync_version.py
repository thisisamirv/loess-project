...
def update_cargo_toml_dependencies(cargo_toml_path: Path, version: str, quiet: bool = False) -> bool:
    ...
    # Update loess-rs/fastLoess dependencies
    # Match: crate = { ... version = "old_version" }
    patterns = [
        r'(loess-rs\s*=\s*{[^}]*version\s*=\s*")[^"]+(")',
        r'(fastLoess\s*=\s*{[^}]*version\s*=\s*")[^"]+(")',
    ]
    ...
    if count_total == 0:
        if not quiet:
             print(f"Warning: No loess-rs/fastLoess dependency patterns found in {cargo_toml_path}")
        return False
...
def update_package_json(package_path: Path, version: str, quiet: bool = False) -> bool:
    ...
    # Match: "fastloess-.*": "^X.Y.Z" (in optionalDependencies)
    # matching specifically fastloess- prefixed packages to avoid touching others
    dep_pattern = r'("fastloess-[a-z0-9-]+"\s*:\s*"\^)[^"]+(")'
