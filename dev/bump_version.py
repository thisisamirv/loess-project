...
Usage:
    python dev/bump_version.py --version 1.2.0
    python dev/bump_version.py --version 1.2.0 --dry-run
"""
...
# Cargo.toml files with a `fastLoess = { path = "../../crates/fastLoess",
...
def build_targets(
    ...
    targets.append(
        (
            "crates/fastLoess/Cargo.toml",
            re.compile(
                r'(loess-rs = \{ path = "\.\./loess-rs", version = ")\d+\.\d+(")'
            ),
            rf"\g<1>{new_major_minor}\g<2>",
            1,
        )
    )
    fastloess_dep_pattern = re.compile(
        r'(fastLoess = \{ path = "\.\./\.\./crates/fastLoess", version = ")\d+\.\d+(")'
    )
    for rel in FASTLOESS_DEP_FILES:
        targets.append((rel, fastloess_dep_pattern, rf"\g<1>{new_major_minor}\g<2>", 1))

    targets.append(
        (
            "bindings/python/python/fastloess/__version__.py",
    ...
    targets.append(
        (
            "bindings/nodejs/package.json",
            re.compile(r'("fastloess-[\w-]+": "\^)\d+\.\d+\.\d+(")'),
    ...
    targets.append(
        (
            "bindings/julia/julia/Project.toml",
            re.compile(r'^fastloess_jll = "\d+\.\d+\.\d+"$', re.MULTILINE),
            f'fastloess_jll = "{new_version}"',
            1,
        )
    )
...
def main() -> int:
    ...
    parser.add_argument("--version", required=True, help="New version, e.g. 1.2.0")
    parser.add_argument(
