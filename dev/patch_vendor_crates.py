...
def main() -> int:
    ...
    # Patch fastLoess
    fast_loess_toml = args.vendor_dir / "fastLoess" / "Cargo.toml"
    if fast_loess_toml.exists():
        if not args.quiet:
            print(f"Patching {fast_loess_toml}")
        patch_crate_toml(
            fast_loess_toml,
            workspace,
            remove_gpu=True,
    ...
    # Patch loess
    loess_toml = args.vendor_dir / "loess" / "Cargo.toml"
