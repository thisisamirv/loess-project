...
MODULE_PATH = "github.com/thisisamirv/loess-project/bindings/go/fastloess"
...
def skip_reason(snippet: Snippet) -> str | None:
    ...
    if not (GO_MODULE_DIR / "go.mod").exists():
        return "bindings/go/fastloess/go.mod not found"
    return None
...
def _ensure_go_snippet_module(names: list[str], snippets: list[Snippet]) -> None:
    ...
    cgo compilation/linking of the shared fastloess binding are paid once
