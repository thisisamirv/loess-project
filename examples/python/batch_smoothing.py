...
def main():
    print("=== fastloess Batch Smoothing Example ===")
    ...
    # Basic Smoothing
    plt.plot(x, res_basic.y, 'r-', linewidth=2, label=f'Basic LOESS (Non-robust)')
    
    # Robust Smoothing
    plt.plot(x, res_robust.y, 'g-', linewidth=2.5, label='Robust LOESS (3 iters)')
    
    # Confidence Intervals
    if res_intervals.confidence_lower is not None and res_intervals.confidence_upper is not None:
        plt.fill_between(
            x, 
    ...
    plt.title("fastloess: Robust Batch Smoothing with Intervals")
    plt.xlabel("X Axis")
    plt.ylabel("Y Axis")
    plt.legend()
