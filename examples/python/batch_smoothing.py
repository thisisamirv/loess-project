...
def main():
    ...
    res_basic = Loess(iterations=0, fraction=0.05).fit(x, y)
    ...
    res_cv = Loess(cv_fractions=cv_fractions, cv_method="kfold", cv_k=5).fit(x, y)
    ...
    # Basic Smoothing
    plt.plot(x, res_basic.y, "r-", linewidth=2, label="Basic LOESS (Non-robust)")
