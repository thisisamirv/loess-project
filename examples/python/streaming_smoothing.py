...
def main():
    print("=== fastloess Streaming Mode Example ===")
    ...
    start = time.time()
    print("Running Batch LOESS (Parallel)...")
    res_batch = Loess(fraction=0.01).fit(x, y)
    ...
    start = time.time()
    print("Running Streaming LOESS (Chunked)...")
    chunk_size = 10_000
    model = StreamingLoess(fraction=0.01, chunk_size=2000, overlap=200, parallel=True)
    ...
    plt.title(f"fastloess: Streaming Smoothing on {n_points} points")
    plt.xlabel("X Axis")
    plt.ylabel("Y Axis")
    plt.legend()
