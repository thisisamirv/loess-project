...
import fastloess
from fastloess import smooth_online
import os
...
def main():
    ...
    # Online Results
    plt.plot(x, res_full.y, 'r-', linewidth=2, label='Online LOESS (Full)')
    plt.plot(x, res_inc.y, 'b-', linewidth=1.5, alpha=0.7, label='Online LOESS (Incremental)')
    
    # Highlight a zoom area to show the windowing effect
    plt.axvspan(400, 500, color='yellow', alpha=0.1, label='Zoom Area')

    plt.title("fastloess: Real-time Online Smoothing (Sliding Window)")
