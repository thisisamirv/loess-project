...
function example_2_sensor_data_simulation()
	...
	model = OnlineLoess(fraction = 0.4, iterations = 3, robustness_method = "bisquare",
		window_capacity = 12)
...
function example_3_outlier_handling()
	...
	for method ∈ ["bisquare", "talwar"]
		model = OnlineLoess(fraction = 0.5, iterations = 5, robustness_method = method,
			window_capacity = 6)
		result = add_points(model, x, y)
	...
	for i ∈ eachindex(x_all)
		...
		if length(result.y) > 0
			@printf("  %4d %4.0f %6.0f %10.2f %-22s\n", i, x_all[i], y_all[i],
				result.y[end], "Window full (sliding)")
		else
...
function example_8_update_modes()
	...
	for mode ∈ ["full", "incremental"]
		model = OnlineLoess(fraction = 0.5, iterations = 2, update_mode = mode,
	...
	result.iterations_used >= 0 && @printf("  iterations_used: %d\n", result.iterations_used)
	println()
end
