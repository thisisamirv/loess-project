...
function example_3_outlier_handling()
	...
	for method ∈ ["bisquare", "talwar"]
		model = OnlineLoess(fraction = 0.5, iterations = 5, robustness_method = method, window_capacity = 6)
...
function example_5_memory_bounded_processing()
	...
	for milestone ∈ [200, 400, 600, 800, 1000]
		milestone <= n_out && @printf("  Processed: %4d pts | smoothed=%.2f\n", milestone, smoothed[milestone])
	end
...
function example_6_sliding_window_behavior()
	...
	for i ∈ eachindex(x_all)
		...
		if r !== nothing
			@printf("  %4d %4.0f %6.0f %10.2f %-22s\n", i, x_all[i], y_all[i], r.smoothed, "Window full (sliding)")
		else
			@printf("  %4d %4.0f %6.0f %10s %-22s\n", i, x_all[i], y_all[i], "-", "Filling ($i/4)")
		end
	end
...
function example_8_update_modes()
	...
	for mode ∈ ["full", "incremental"]
		model = OnlineLoess(fraction = 0.5, iterations = 2, update_mode = mode, min_points = 5, window_capacity = 15)
		smoothed = filter(!isnan, add_all_points(model, x, y))
...
function example_9_advanced_online_options()
	...
	y = 2 .* x .+ 1
	model = OnlineLoess(
		fraction = 0.5, iterations = 2, degree = "quadratic", scaling_method = "mar",
		boundary_policy = "reflect", zero_weight_fallback = "return_original",
		distance_metric = "chebyshev", auto_converge = 1e-3, return_robustness_weights = true,
		min_points = 5, window_capacity = 15,
	)
	smoothed = filter(!isnan, add_all_points(model, x, y))
