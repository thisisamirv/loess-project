...
function example_2_robust_with_outliers()
	...
	result = fit(Loess(fraction = 0.5, iterations = 5, robustness_method = "bisquare",
			return_robustness_weights = true, return_residuals = true), x, y)
...
function example_3_uncertainty_quantification()
	...
	result = fit(Loess(fraction = 0.5, iterations = 3, confidence_intervals = 0.95,
			prediction_intervals = 0.95), x, y)
	println("  x  y_smooth  conf_low  conf_high  pred_low  pred_high")
	for i ∈ eachindex(result.y)
		@printf("  %d  %.4f  %.4f  %.4f  %.4f  %.4f\n",
			Int(result.x[i]), result.y[i],
...
function example_4_cross_validation()
	...
	y = 2 .* x .+ 1 .+ sin.(x .* 0.5)
	result = fit(Loess(cv_fractions = [0.2, 0.3, 0.5, 0.7], cv_method = "kfold", cv_k = 5,
			iterations = 2, return_diagnostics = true), x, y)
...
function example_5_complete_diagnostics()
	...
	result = fit(Loess(fraction = 0.5, iterations = 3, confidence_intervals = 0.95,
			...
			return_residuals = true, return_robustness_weights = true), x, y)
	...
	result.robustness_weights !== nothing && @printf("  rob_weight[1]: %.4f\n", result.robustness_weights[1])
	println()
end
...
function example_7_robustness_methods()
	...
	for method ∈ ["bisquare", "huber", "talwar"]
		result = fit(Loess(fraction = 0.5, iterations = 5, robustness_method = method,
				return_robustness_weights = true), x, y)
		println("  $method:")
		println("    Smoothed: [$(join(round.(result.y, digits=2), ", "))]")
		if result.robustness_weights !== nothing
			println("    Weights:  [$(join(round.(result.robustness_weights, digits=3), ", "))]")
		end
	end
	println()
end
...
function example_12_polynomial_degrees()
	...
	for deg ∈ ["constant", "linear", "quadratic", "cubic", "quartic"]
		...
		@printf("  %s: y[1]=%.3f, iterations_used=%d\n", deg, result.y[1], result.iterations_used)
	end
	println()
end
...
function example_14_surface_modes_and_se()
	...
	r = fit(Loess(fraction = 0.5, surface_mode = "direct", return_se = true,
			confidence_intervals = 0.95, prediction_intervals = 0.95), x, y)
	...
	r.standard_errors !== nothing && @printf("    standard_errors[1]: %.4f\n", r.standard_errors[1])
	...
	@printf("    y[1]: %.3f\n", r2.y[1])
	r2.standard_errors !== nothing && @printf("    standard_errors[1]: %.4f\n", r2.standard_errors[1])
	println()
end
...
function example_16_loocv_and_auto_converge()
	...
	r_kfold = fit(Loess(cv_fractions = [0.2, 0.4, 0.6], cv_method = "kfold", cv_k = 5), x, y)
...
function example_17_interpolation_tuning()
	...
	r_se = fit(Loess(fraction = 0.5, surface_mode = "interpolation", return_se = true), x, y)
